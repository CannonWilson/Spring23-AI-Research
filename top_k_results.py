...
for mode in MODES:
    ...
    ds_values = [] #torch.empty(IMGS_PER_CLASS).to(device)
    confidences = [] #torch.empty(IMGS_PER_CLASS).to(device)
    sexes = [] #torch.empty(IMGS_PER_CLASS, dtype=torch.int8).to(device)
    smiles = [] #torch.empty(IMGS_PER_CLASS, dtype=torch.int8).to(device)
    correctness = [] #torch.empty(IMGS_PER_CLASS, dtype=torch.int8).to(device)
    img_feature_stack = [] #torch.empty((IMGS_PER_CLASS, EMBEDDING_DIM)).to(device)
    ...
    with torch.no_grad():
        ...
        for i, (image, label) in enumerate(test_loader):
            ...
            cur_sex = 1 if sex == "male" else 0
            cur_smi = 1 if smiling == "smile" else 0
            sexes.append(cur_sex)
            smiles.append(cur_smi)
            # sexes[cur_idx] = 1 if sex == "male" else 0 # 0 values allow total (male) count to be found with a simple sum
            # smiles[cur_idx] = 1 if smiling == "smile" else 0       
            ...
            # Increase index of imgs for current class
            cur_idx += 1
    ...
    conf_sorted_idxs, ds_sorted_idxs = np.argsort(confidences), np.argsort(ds_values)
    conf_sorted_sexes, ds_sorted_sexes = np.array(sexes)[conf_sorted_idxs], np.array(sexes)[ds_sorted_idxs]
    conf_sorted_smiles, ds_sorted_smiles = np.array(smiles)[conf_sorted_idxs], np.array(smiles)[ds_sorted_idxs]
    conf_sorted_frac_male, ds_sorted_frac_male = np.empty(IMGS_THIS_CLASS), np.empty(IMGS_THIS_CLASS)
    conf_sorted_frac_smiles, ds_sorted_frac_smiles = np.empty(IMGS_THIS_CLASS), np.empty(IMGS_THIS_CLASS)
    
    for num_people in range(IMGS_THIS_CLASS):
        conf_num_males, ds_num_males = conf_sorted_sexes[:num_people].sum(), ds_sorted_sexes[:num_people].sum()
        conf_num_smiles, ds_num_smiles = conf_sorted_smiles[:num_people].sum(), ds_sorted_smiles[:num_people].sum()
    ...
    assert conf_num_males == ds_num_males # make sure that the final tally for the number of males is the same
    assert conf_num_smiles == ds_num_smiles # make sure that the final tally for the number of smiles is the same
